"""
K-League Pass Prediction - Ensemble Model Inference

앙상블 모델(XGBoost + LightGBM)로 테스트 데이터 예측 및 제출 파일 생성
최적 가중치 자동 적용
"""
...
def load_ensemble_model(model_path='ensemble_model.pkl'):
    """앙상블 모델 로딩"""
    print(f"📂 앙상블 모델 로딩: {model_path}")
...
def predict_ensemble(X, models, weights):
    """앙상블 예측"""
    predictions = []
...
def predict_test_data(ensemble_model, preprocessor, feature_config,
    ...
    for idx, row in tqdm(test_index.iterrows(), total=len(test_index)):
        ...
        try:
            ...
            # period_id: 1로 가정 (또는 평균값)
            # is_home: 1로 가정 (또는 0.5)
            ...
            # 앙상블 예측
            pred = predict_ensemble(X, models, weights)
...
def create_submission(test_index, predictions, output_path='submission.csv'):
    """제출 파일 생성"""
    print("\n" + "=" * 80)
    print("  제출 파일 생성")
    print("=" * 80)
...
def main():
    """메인 실행 함수"""
    print("=" * 80)
    print("  K-League Pass Prediction - Ensemble Inference")
    print("=" * 80)
    ...
    # 메타데이터
    val_score = ensemble_model.get('val_score')  # 앙상블 모델에 저장되어 있다면
    ...
    backup_path = f'submission_ensemble_{timestamp}.csv'
