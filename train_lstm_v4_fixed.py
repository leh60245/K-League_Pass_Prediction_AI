"""
K-League Pass Prediction - PyTorch LSTM/GRU 학습 파이프라인 (개선 버전)

🔧 주요 개선사항:
1. Loss 계산 로직 수정 (정규화/역정규화 검증)
2. Learning Rate 대폭 감소 (1e-3 → 1e-4)
3. Batch Normalization 추가
4. Gradient Clipping 추가
5. 더 강력한 Regularization

작성일: 2025-12-18 (Fixed)
"""
...
class SoccerRNN(nn.Module):
    """
    Embedding + Input Projection + GRU/LSTM 기반 패스 예측 모델 (개선 버전)
    ...
    def __init__(self,
        ...
        self.bn_numerical = nn.BatchNorm1d(num_numerical_features)
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim // 4, 2)  # (target_x, target_y)
        )
    ...
    def forward(self, num_feat, cat_feat):
        ...
        """
        batch_size, seq_len, num_feat_dim = num_feat.shape

        # Batch Norm for numerical (reshape to apply BN)
        num_feat_flat = num_feat.view(-1, num_feat_dim)  # (batch*seq, num_feat)
        num_feat_bn = self.bn_numerical(num_feat_flat)
        num_feat_bn = num_feat_bn.view(batch_size, seq_len, num_feat_dim)
        ...
        if embedded:
            ...
            x = torch.cat([num_feat_bn, embedded], dim=-1)  # (batch, seq_len, input_dim)
        else:
            x = num_feat_bn
...
class EuclideanDistanceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        pred_real[:, 0] = pred_real[:, 0] * 105.0
        pred_real[:, 1] = pred_real[:, 1] * 68.0

        target_real = target.clone()
        target_real[:, 0] = target_real[:, 0] * 105.0
        target_real[:, 1] = target_real[:, 1] * 68.0

        # 유클리드 거리
        distances = torch.sqrt(torch.sum((pred_real - target_real) ** 2, dim=1) + 1e-8)
...
def train_one_epoch(model, dataloader, criterion, optimizer, device, clip_grad=True):
    """1 에포크 학습 - Gradient Clipping 추가"""
    model.train()
    total_loss = 0.0
...
def main():
    print("=" * 80)
    print("  PyTorch LSTM/GRU - V4 Wide Format 학습 (개선 버전)")
    print("  🔧 Learning Rate 감소 + 강화된 Regularization")
    print("=" * 80)
    print()

    # 하이퍼파라미터 (개선)
    K = 20
    BATCH_SIZE = 64  # 128 → 64 (더 안정적인 학습)
    HIDDEN_DIM = 128  # 256 → 128 (과적합 방지)
    NUM_LAYERS = 2
    DROPOUT = 0.4  # 0.3 → 0.4 (더 강한 정규화)
    LEARNING_RATE = 1e-4  # 1e-3 → 1e-4 (핵심 개선!)
    NUM_EPOCHS = 100  # 50 → 100 (충분한 학습)
    EARLY_STOPPING_PATIENCE = 15  # 10 → 15
    USE_LSTM = True  # False → True (LSTM 시도)

    print(f"🔧 하이퍼파라미터 (개선):")
    print(f"   - Sequence Length: {K}")
    print(f"   - Batch Size: {BATCH_SIZE} (↓ 더 안정적)")
    print(f"   - Hidden Dim: {HIDDEN_DIM} (↓ 과적합 방지)")
    print(f"   - Num Layers: {NUM_LAYERS}")
    print(f"   - Dropout: {DROPOUT} (↑ 강한 정규화)")
    print(f"   - Learning Rate: {LEARNING_RATE} (↓↓ 핵심 개선!)")
    ...
    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-3)  # weight_decay 증가
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        # Train
        train_loss = train_one_epoch(model, train_loader, criterion, optimizer, device, clip_grad=True)
    ...
    print(f"✅ 모델 저장: lstm_model_v4_fixed_best.pth")

    print("\n📊 성능 비교:")
    print(f"   - LightGBM V4 (5-Fold): ~1.5m")
    print(f"   - LSTM/GRU V4 (Original): 15.98m")
    print(f"   - LSTM/GRU V4 (Fixed): {best_val_loss:.4f}m")

    if best_val_loss < 2.0:
        print("\n🎉 매우 우수한 성능! LightGBM과 경쟁 가능합니다!")
    elif best_val_loss < 5.0:
        print("\n✅ 좋은 개선! 추가 튜닝으로 더 개선 가능합니다.")
    else:
        print("\n📈 추가 분석 필요 - 데이터 또는 모델 구조 재검토")

    print("\n" + "=" * 80)
    print("개선 사항:")
    print("   ✅ Learning Rate: 1e-3 → 1e-4 (10배 감소)")
    print("   ✅ Batch Size: 128 → 64 (더 안정적)")
    print("   ✅ Hidden Dim: 256 → 128 (과적합 방지)")
    print("   ✅ Dropout: 0.3 → 0.4 (강한 정규화)")
    print("   ✅ Batch Normalization 추가")
    print("   ✅ Gradient Clipping 추가")
    print("   ✅ 더 깊은 Output Head")
    print("=" * 80)
